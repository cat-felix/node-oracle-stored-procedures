...
const procs = require("./api-procs.js");
//	JSON response
const pattern = {
	...
	oradb.createPool({
		...
		.get("/api/:query", (req, res) => {
			dbpool.getConnection(async(err, dbconn)=>{
				...
				let _keys = Object.keys(cfg); // ["departments", "goods", "insertMessage"]
				let out	= {}, promises = [];
				...
				for (let i = 0; i < _keys.length; i++) {
					for (let j = 0; j < cfg[_keys[i]].length; j++) {
						...
						switch (_keys[i]) {
							...
							// stored procedures
							case "insertMessage":
								promises.push(procs.ora_exec_im(dbconn, cfg[_keys[i]][j], out[_keys[i]][j]));
							break;
							default:
								utils._trace(utils.f_exc_log, "Query or stored procedure is not found: "+_keys[i], cfg);
							break;
						}
					}
				}
